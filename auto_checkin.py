...
def send_push(session: requests.Session, title: str, body: str = "", level: str = "passive"):
    url = (
        PushClient(PUSH_URL)
        .add_path(title, body)
...
with requests.Session() as s:
    try:
        ...
        if not (login_resp.ok and login_data.get("ret") == 1):
            send_push(s, "69云 - 登录失败", login_msg or f"状态码: {login_resp.status_code}", level="passive")
            raise SystemExit(f"[ERROR] login failed, Exception: {login_msg or login_resp.status_code}")
    except Exception as exc:
        send_push(s, "69云 - 登录失败", str(exc), level="passive")
